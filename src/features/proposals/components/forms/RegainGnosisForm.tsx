...
/**
 * RegainGnosisForm — Step 2 form for the "regain_gnosis" action type.
 *
 * Fields:
...
interface RegainGnosisFormProps {
  character: CharacterDetailResponse;
  onNext: (data: Record<string, unknown>) => void;
}

export function RegainGnosisForm({ character, onNext }: RegainGnosisFormProps) {
  const { goBack, state, setNarrative } = useWizard();
  const savedData = (state.formData["regain_gnosis"] ?? {}) as Partial<FormValues>;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6" noValidate>
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor="regain-gnosis-narrative" className="text-sm font-semibold text-text-primary">
          Narrative
          <span className="ml-1 text-status-rejected text-xs" aria-hidden="true">*</span>
        </label>
        <p className="text-xs text-text-secondary">
          Describe what your character does to restore their gnosis.
        </p>
        <textarea
          id="regain-gnosis-narrative"
          {...register("narrative")}
          rows={4}
          placeholder="I retreat to my sanctum and meditate, reconnecting with the resonance of the world..."
          className="
            ...
            resize-y min-h-[96px]
          "
          aria-describedby={errors.narrative ? "regain-gnosis-narrative-error" : undefined}
          aria-required="true"
        />
        {errors.narrative && (
          <p id="regain-gnosis-narrative-error" className="text-xs text-status-rejected" role="alert">
