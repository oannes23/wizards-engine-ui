...
- Name: auth-login.spec.ts >> Auth: magic link login >> invite code join form: fill display name and character name, submit, redirects to feed
- Location: e2e/auth-login.spec.ts:68:7
...
Locator: getByRole('heading', { name: 'Join the Game' })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Join the Game' })
