...
- Name: a11y.spec.ts >> Accessibility: critical pages >> login page (/login) has no critical violations
- Location: e2e/a11y.spec.ts:71:7
...
```ts
  ...
  3   | import { bypassAuth } from "./helpers";
  4   | 
  5   | /**
  6   |  * Accessibility audit tests using axe-core.
  7   |  *
  8   |  * Strategy: best-effort for MVP. We run axe on critical pages and report
  9   |  * violations without failing the suite on every violation — the goal is to
